...
Each officer has a `name`, `role`, `grade`, `image` and `bio`. Put officer photos in `src/assets/team/` and set `"image"` to the file name. Entries with `"featured": true` (the co-presidents) get their own row of bigger cards at the top. Add or remove entries and the page adjusts. `contacts` is who questions go to (the co-presidents): their emails show in the footer and under the class code. The advisor card uses `advisor.name`, `advisor.email` and `advisor.image`. Without an `image` the card shows the advisor's initials; `email` is shown in the footer when filled in.
